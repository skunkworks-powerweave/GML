...
export default async function ObservationGateLayout({ children }: { children: ReactNode }) {
  ...
  // every request; the "/observation" fallback covers the case where it is absent.
  const requested = (await headers()).get("x-pathname") ?? "/observation";
  await assertSectionGate(session.user.id, "observation", requested);
