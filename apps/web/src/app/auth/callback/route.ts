...
/**
 * Same-site absolute paths only.
 *
 * `next` arrives in a URL that was mailed to the user, which makes it
 * attacker-supplied in the most effective possible way: a link that really does
 * authenticate them, and then forwards them somewhere else. Reject protocol-
 * relative (`//evil`) and backslash (`/\evil`) forms, which browsers resolve to
 * a foreign origin.
 */
