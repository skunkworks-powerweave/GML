...
docker compose exec -T minio sh -c "mc alias set local http://localhost:9000 \$MINIO_ROOT_USER \$MINIO_ROOT_PASSWORD >/dev/null && mc mirror --overwrite local/ /tmp/mirror >/dev/null" || true
docker compose cp minio:/tmp/mirror "$OBJ_DIR/${DATE_STAMP}/" || echo "[backup] minio mirror skipped (likely empty)"
